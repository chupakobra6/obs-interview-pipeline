...
| REQ-011 | `готово` | S007, S008 | OBS-команда Harvest не выполняет медленный whole-file Silero gate и не дублирует production model/decode/Metal/ru/post-filter настройки. | ASR descriptor не содержит whole-file speech gate, но подтверждает тот же model q5_0, Metal, ru, beam 5 и bounded long-form preparation; transcript проходит Harvest post-filter. | E2E manifest: `speech_gate=0`, `trusted_long_form`, q5_0/Metal/ru/beam 5/`terminal-exact-v1`; короткий ASR 5.28 s, реальный 16-минутный — 37.82 s. |
