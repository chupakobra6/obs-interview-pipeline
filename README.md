...
После остановки записи OBS проект показывает короткий macOS-диалог: обработать запись или оставить как есть, удалять ли исходник после успеха и сводить ли аудиодорожки. Завершённые записи SobesTech импортируются автоматически. Запись превращается в компактное HEVC-видео и Markdown-расшифровку; удаление возможно только после полной проверки результата, а оригиналы SobesTech всегда сохраняются.
...
3. LaunchAgent запускает worker с интерактивным QoS: явная пользовательская обработка не получает системный background throttle. Worker сначала обрабатывает медиаконтейнер, затем вызывает канонический адаптивный `telegram-harvest transcribe-file`; тяжёлые VideoToolbox и Whisper-этапы не конкурируют за ресурсы Mac.
4. Готовый source HEVC `1512x982@30` копируется без повторного video encode; для старого или иного входа включается приоритетный по скорости VideoToolbox transcode и только необходимые `scale`/`fps` filters. Битрейт рассчитывается из размера и длительности source, ограничивается через `maxrate` и оставляет запас на muxing: итог не может превышать 80% исходного размера.
5. По умолчанию все source audio tracks сохраняются раздельно и кодируются в AAC с target 96 Кбит/с. Галочка merge сводит их в одну AAC-дорожку. Проверка подтверждает ожидаемое число streams, codec/bitrate, разрешение, fps, длительность и установленный предел размера.
...
`com.igor.sobestech-importer` следит за каталогом `~/Library/Application Support/com.pers0na2.identityproxy/interview_recordings` и дополнительно проверяет его каждые 30 секунд. Импортёр принимает только пару JSON/MP4 со статусом `completed`, без ошибки и с совпадающим размером; после последнего изменения обоих файлов ждёт 15 секунд и проверяет медиапотоки.

MP4 атомарно копируется в `~/Movies` под именем времени записи и только эта staging-копия ставится в очередь с последующим удалением после успеха. Исходный MP4 SobesTech не изменяется. Receipt и поиск задания в `queue/done/failed` обеспечивают exactly-once постановку даже после перезапуска. Ошибочный неизменившийся manifest получает не более трёх попыток с backoff, после чего сохраняется терминальный статус и не блокирует другие записи; ошибка показывается уведомлением и остаётся в `logs/sobestech-importer-error.log`.
